...
from ..processor.process import TOPIC_LABELS, TECH_QUERIES
...
def cluster_articles(df_tech: pd.DataFrame, tech_embeddings: np.ndarray) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    cluster_sim = cosine_similarity(centers, np.array([
        np.zeros(centers.shape[1])
    ]))

    query_emb_placeholder = tech_embeddings[:len(TECH_QUERIES)] if len(tech_embeddings) >= len(TECH_QUERIES) else tech_embeddings
    return df_tech, centers, kmeans


def select_highlights(df_tech: pd.DataFrame, tech_embeddings: np.ndarray, cluster_centers: np.ndarray) -> pd.DataFrame:
    highlight_rows = []
    for c in range(N_CLUSTERS):
        ...
        highlight_rows.append(cluster_df.nlargest(TOP_NEWS_PER_CLUSTER, "dist_to_center"))
